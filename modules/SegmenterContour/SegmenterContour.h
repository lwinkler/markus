...
public:
	SegmenterContourParameterStructure(const ConfigReader& x_confReader) : ModuleParameterStructure(x_confReader)
	{
		...
		m_list.push_back(new ParameterString("features",     "x,y,width,height",           &features,   "List of features to extract, separated with ',' possible: x,y,width,height,ellipse_{angle,ratio,width,height}"));
